...
def questionTwo(event):
    ...
    try:
        import stepBrotherImSTACK as stackModule
    except:
        q2.destroy()
    ...
    delButton = Button(q2, text="delete somebody")
    delButton.grid(row=1, column=5)

    def delInStack(event):
        stack.delete(enter.get())
        enter.delete(0, END)
        pass
...
# Создать простую очередь строк на основе статического массива. Реализовать методы : Добавить элемент, удалить элемент, длина очереди.


def QuestionThree(event):
    q3 = Tk()
    ...
    addButton = Button(q3, text="add")
    addButton.grid(row=1, column=2)
    addButton.bind("<Button-1>", adding)
    addButton.place()
    ...
    delButton = Button(q3, text="del")
    delButton.grid(row=1, column=3)
    delButton.bind("<Button-1>", delit)
    delButton.place()

    def see(event):
        enter.delete(0, END)
        a=qw.get()
        if(a!=None):enter.insert(0,a)
        else:enter.insert(0,"!empty")
        pass

    seeButton = Button(q3, text="see")
    seeButton.grid(row=1, column=4)
    seeButton.bind("<Button-1>", see)
    seeButton.place()

    pass


questionThreeButton = Button(root, text="example 3")
questionThreeButton.grid(row=2, column=2)
questionThreeButton.bind("<Button-1>", QuestionThree)
questionThreeButton.place()
